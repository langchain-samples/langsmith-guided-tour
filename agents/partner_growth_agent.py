"""Partner growth deep agent — merchant-facing sales agent for a retail-media platform.

Looks up merchant accounts, scores performance-marketing fit, and researches
merchants on the web. Mirrors research_agent.py's eval-safe shape: no HITL, no
FilesystemBackend, fresh MemorySaver per call.
"""
...
def build_partner_growth_agent():
    """Return a fresh partner-growth deep agent."""
    research_subagent = {
        "name": "research-agent",
        "description": "Delegate merchant / category research tasks. Give one topic at a time.",
        "system_prompt": (
            f"You are a research assistant for a retail-media sales team. Today is "
            f"{datetime.now().strftime('%Y-%m-%d')}.\n"
            "Use web_search to gather retail-media category trends, benchmarks, and "
            "competitive / industry context — not facts about a specific merchant by name "
            "(those come from the merchant profile). Limit to 3 search calls. Cite sources."
        ),
        "tools": [web_search],
    }
    ...
    return create_deep_agent(
        ...
        system_prompt=(
            "You are a partner growth assistant for a retail-media and performance-marketing "
            "platform. Help sales reps qualify merchants, prep for calls, and draft outreach by "
            "looking up merchant profiles and scoring performance-marketing fit. Merchant-specific "
            "facts and recent signals come from get_merchant_profile; delegate retail-media "
            "category, benchmark, and competitive research to the research-agent rather than "
            "web-searching individual merchants by name."
        ),
